...
function Layout({ children }) {
  ...
  const drawer = (
    <>
      <Box sx={{ display: 'flex', justifyContent: 'flex-start', px: 1, pt: 1 }}>
        {/* language toggle placed at the top-left corner of the menu */}
  ...
  // close temporary drawer when route changes on mobile
  React.useEffect(() => {
        ...
        <Toolbar sx={{ minHeight: { xs: 56, md: 64 } }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <IconButton color="inherit" onClick={() => {
              // Open account manager only for users with manage_accounts permission
              ...
              if ((acct && acct.permissions && acct.permissions.manage_accounts) || isCoreAdmin) {
                ...
                // show brief feedback so user understands why it didn't open
