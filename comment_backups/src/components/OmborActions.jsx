...
export default function OmborActions() {
  ...
  const filtered = useMemo(() => {
    ...
    return logs.filter(l => {
      // normalize source/kind/action for flexible matching
      const src = ((l.source || l.kind || l.action || '') + '').toLowerCase()
      if (source !== 'all') {
        if (!src.includes(String(source).toLowerCase())) return false
      }

      // compute timestamp from ts OR date (ISO YYYY-MM-DD or full)
      let entryTs = null
      if (l.ts) entryTs = Number(l.ts)
        ...
        // fallback: if only time is present, ignore period filtering
        entryTs = null
      }
  ...
  const getAmountDisplay = (l) => {
    // Prefer showing USD raw amounts if currency is USD, otherwise show UZS totals
