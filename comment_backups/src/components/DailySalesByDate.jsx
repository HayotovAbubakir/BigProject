...
// Component: shows sales for selected date (defaults to today)
export default function DailySalesByDate({ selectedDate: propDate, onDateChange }) {
  ...
  const salesForDate = useMemo(() => {
    ...
    return state.logs.filter((l) => {
      // Only show SELL logs by default (case-insensitive)
  ...
  const totalUzs = useMemo(() => {
    return salesForDate.reduce((acc, l) => {
      // Prefer canonical total_uzs, fallback to amount_uzs or amount converted via current rate
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 2 }}>
                      <Box sx={{ minWidth: 0, flex: 1 }}>
                        ...
                        <Box sx={{ display: 'flex', gap: 1, mt: 1, flexWrap: 'wrap' }}>
                          ...
                          {(() => {
                            // Compute unit price fallback when missing: amount/qty
                          ...
                          {(() => {
                            ...
                            // fallback: compute amount = unit * qty
                            const qty = Number(l.qty || 0)
