...
export default function StoreForm({ open, onClose, onSubmit, initial }) {
  ...
  useEffect(() => {
    ...
    // If there's a saved draft, use it. Fall back to initial/default when there is no saved draft.
    try {
      if (saved) {
        setForm(saved)
        return
  ...
  // persist draft while dialog is open into AppContext (debounced to avoid rapid dispatches)
  useEffect(() => {
    if (!open) return
    const key = getDraftKey()
    const t = setTimeout(() => {
  ...
  const submit = () => {
    // If item is in USD and there's a cached conversion in localStorage we can attach a UZS copy
    ...
    onSubmit(payload)
    // clear draft on submit (saved in AppContext)
    try { dispatch({ type: 'CLEAR_DRAFT', payload: { key: getDraftKey() } }) } catch (err) { void err }
    onClose()
  }

  const handleClose = () => {
    // Keep draft on close so user can resume later. Only submit clears the draft.
    onClose()
  }

  // Currency modal confirm handler (render CurrencyModal here)
  const [currencyOpenInternal, setCurrencyOpenInternal] = useState(false)
  function onCurrencyConfirmInternal({ currency }) {
    // Use global exchange rate from menu — do not save local rate here.
