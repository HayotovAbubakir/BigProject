/* ReceiptApp.jsx
 *
 * A polished React component showing a receipt with:
 * - Dark mode toggle (Tailwind 'class' strategy). Persisted to localStorage.
 * - Receipt calculations for mixed-currency items (USD / UZS) with configurable exchange rate.
 * - Animated cosmic background, glass card UI, Framer Motion top alert.
 * - Helper functions: formatCurrencyUZS(number), roundUsd(number).
 *
 * How to use:
 * 1. Drop this file into your React project (e.g. src/components/ReceiptApp.jsx).
 * 2. Tailwind CSS must be configured in your project using the 'class' dark mode strategy:
 *      // tailwind.config.js
 *      module.exports = {
 *        darkMode: 'class',
 *        // ...rest
 ...
 *
 * Notes:
 * - The component accepts an optional `items` prop (array of { name, qty, unitPrice, currency }).
 *   If not provided it uses an internal demo dataset for quick testing.
 * - Exchange rate defaults to 13000 UZS / 1 USD, and is configurable (persisted to localStorage).
 */

import React, { useEffect, useMemo, useRef } from 'react'
import { useApp } from '../context/AppContext'
// framer-motion is optional. We dynamically import it at runtime so that
// the dev server/build won't fail if the package isn't installed.
// If it's absent we gracefully fall back to plain elements.
// NOTE: to enable animations, install framer-motion: `npm install framer-motion`

// (framer-motion optional dynamics removed) the component now uses CSS fallbacks
...
export default function ReceiptApp({ items: propItems }) {
  // no dynamic framer-motion import — we use CSS animations/fallbacks here

  // We intentionally avoid requiring framer-motion at build-time.
  // If framer-motion is installed it will be loaded via dynamic import
  // into `fm` and we could opt-in to richer animations.
  ...
  // rate is kept in-memory; if you want to persist it to Firestore use AppContext.exchangeRate
  ...
  // alertVariants previously used with framer-motion; removed in favor of CSS
