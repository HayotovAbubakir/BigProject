...
export default function CreditForm({ open, onClose, onSubmit, initial }) {
  ...
  useEffect(() => {
    ...
    // no per-dialog cached rate; global menu rate will be used
  }, [initial])
  ...
  const submit = () => {
    ...
    // For UZS store the UZS amount; for USD keep the original USD amount only
    // so that UZS equivalent can be recomputed when the exchange rate changes.
    if (payload.currency === 'USD') {
      // ensure amount is numeric but do NOT store amount_uzs so display uses current rate
  ...
  // saveRate removed; global menu control stores exchange rate
