...
export default function WarehouseForm({ open, onClose, onSubmit, initial }) {
  ...
  useEffect(() => {
    ...
    // Load saved draft if present; fall back to initial/default when none.
    try {
      if (saved) {
        setForm(saved)
        return
  ...
  // save draft into AppContext while open
  useEffect(() => {
    if (!open) return
    const key = getDraftKey()
    const t = setTimeout(() => {
  ...
  const handleClose = () => {
    // Keep draft on close; clear only on submit
    onClose()
  }
  ...
  return (
    <Dialog open={open} onClose={handleClose} fullWidth maxWidth="sm">
      ...
      <CurrencyModal open={currencyOpen} onClose={() => setCurrencyOpen(false)} current={form.currency} onConfirm={({ currency }) => {
        // Keep numeric cost unchanged; only update currency flag. Global rate is used from menu.
