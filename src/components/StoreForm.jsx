  ...
  return (
    ...
    <DialogTitle sx={{ fontSize: { xs: '0.95rem', md: '1.15rem' }, p: { xs: 1.5, md: 2 } }}>{initial ? "Tahrirlash" : "Qo'shish"}</DialogTitle>
        <DialogContent sx={{ p: { xs: 1.5, md: 2 }, overflowWrap: 'break-word', pt: { xs: 1, md: 1.5 }, overflowX: 'visible' }}>
          ...
          {isElectrode && (
            <CurrencyField label="Narxi (1 pachka)" fullWidth margin="dense" value={form.price_pack} onChange={(v) => setForm(prev => ({ ...prev, price_pack: v === null ? '' : v, price: v === null ? '' : v }))} currency={form.currency} />
          )}
          {isStone && (
