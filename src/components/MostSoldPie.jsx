...
const MostSoldPie = ({
  ...
  const processed = data.map((d, i) => ({
    ...d,
    color: d.color || `hsl(${(i * 60) % 360} 70% 55%)`,
  }));
      ...
      >
        ...
        >
          ...
          {data.map((d, i) => {
            ...
            const isFull =
              anglePercent >= 359.999 || Math.abs(value / total - 1) < 1e-9;
          ...
          {centerLabel ? (
            <g>
              <text
                x={cx}
                y={cy - 6}
                fontSize={Math.max(12, size * 0.08)}
                textAnchor="middle"
                fill="#111"
                style={{ fontWeight: 700 }}
              >
